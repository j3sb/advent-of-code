use std::{arch::x86_64::CpuidResult, collections::HashSet, fs, vec};
...
fn add_circuit(circuits: &mut Vec<HashSet<usize>>, connection: &HashSet<usize>) {
    let mut modified = false;
    let mut i = 0;
    while i < circuits.len() {
        if connection.iter().any(|p| circuits[i].contains(p)) {
            // put both together
            circuits[i].extend(connection);
            modified = true;
        }
        i += 1;
    }
...
fn main() {
    ...
    let connections: Vec<HashSet<usize>> = dists
        .iter()
        // .take(num_pairs)
    ...
    let mut last_connection: &HashSet<usize> = &HashSet::new();
    for connection in &connections {
        last_connection = &connection;
        circuits.push(connection.clone());
        get_circuits(&mut circuits);
        if circuits.len() == 1 && circuits[0].len() == points.len() {
            break;
        }
    }

    let prod = last_connection
        .iter()
        .fold(1isize, |acc, i| acc * points[*i].0);
    println!("{prod}");
}
